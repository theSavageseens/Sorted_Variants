...
feature -- commands
	...
	prune_tolerant (val: V; i: INTEGER)
		...
		do
			...
			pruned: (1 <= i and then i <= occurrences (val).count) implies  (model ~ (old model.deep_twin @<< (((old occurrences (val)).deep_twin) [i])))
				-- To Do
	...
	merge (other: SORTED_MAP_ADT [K, V])
		...
		require
			distinct_keys: (model.domain |/\| other.model.domain).is_empty
			-- To Do
			-- Hint: Consider the intersection operator |/\| in the SET class in Mathmodels.
		deferred
		ensure
			merged: ((old model.deep_twin).domain |\/| other.model.domain) ~ model.domain
			-- To Do
